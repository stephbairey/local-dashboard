/* calendar.js — main calendar (hides $-tagged bill events) */
(function(){
  ...
  function row(ev){
    ...
    const locStr  = (ev.location && ev.location.trim()) ? U.linkifyLocation(ev.location.trim()) : "";
    const EH = U.escapeHtml || (s=>String(s).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c])));

    return `
      <div class="cal-row">
        <div class="cal-title">${EH(ev.title || "Untitled")}</div>
  ...
  R("slide-calendar", function renderCalendar(data){
    ...
    try{
      ...
      if (evs.length === 0) {
        el.innerHTML = '<p class="small">Calendar data unavailable.</p>'; return;
      }
